...
public class PremiumCalculationService {
	...
	public PremiumQuote calculateQuote(PolicyPlan plan, BigDecimal coverageAmount, PremiumType premiumType) {
		PremiumQuote quote = computeQuote(plan, coverageAmount, premiumType);
		// A ₹0 premium can never be paid (payments must be >= ₹1 and exact), so a
		// policy priced at ₹0 could never become ACTIVE. Refuse it up front.
		if (quote.finalPremium().compareTo(BigDecimal.ZERO) <= 0) {
			throw new BusinessRuleException(
					"This coverage and payment frequency give a premium of ₹0, which can't be paid. "
							+ "Choose a higher coverage amount or a different payment frequency.");
		}
		return quote;
	}

	/** Same math as {@link #calculateQuote} without the ₹0 guard (used to validate plan setup). */
	public PremiumQuote computeQuote(PolicyPlan plan, BigDecimal coverageAmount, PremiumType premiumType) {
