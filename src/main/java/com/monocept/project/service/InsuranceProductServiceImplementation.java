...
public class InsuranceProductServiceImplementation implements InsuranceProductService {
	...
	public InsuranceProductResponseDTO createProduct(InsuranceProductRequestDTO productRequestDTO) {
		...
		// Honour the requested status (it is a required field on the request);
		// previously every new product was forced active.
		product.setActiveStatus(productRequestDTO.getActiveStatus() == null
				|| productRequestDTO.getActiveStatus());
