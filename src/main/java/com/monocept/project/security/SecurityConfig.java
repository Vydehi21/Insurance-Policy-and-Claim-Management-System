...
public class SecurityConfig {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http)
        ...
        // Requests with no/invalid credentials and role denials raised by the
        // URL rules above now return the standard JSON error body
        // (timestamp, statusCode, errorType, message, path). Previously a
        // missing token produced an empty 403 with no body.
        .exceptionHandling(ex -> ex
                ...
                .accessDeniedHandler((request, response, accessDeniedException) ->
                        resolver.resolveException(request, response, null,
                                new AuthorizationException("You do not have permission to access this resource")))
        )
