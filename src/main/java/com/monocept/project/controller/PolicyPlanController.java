...
public class PolicyPlanController {
    ...
    public ResponseEntity<PaginatedResponseDTO<PolicyPlanResponseDTO>> getAllPlans(
        ...
        // FR-PLN-004: customers only see active plans.
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'INTERNAL_STAFF', 'CUSTOMER')")
    @Operation(summary = "Get Plans By Product ID", description = "Filters the plan schemas to return configurations nested under a common insurance product type, optionally further filtered by active status")
    public ResponseEntity<PaginatedResponseDTO<PolicyPlanResponseDTO>> getPlansByProduct(
        ...
        // Customers only see purchasable plans: active plans under an active
        // product. Inactive plans used to be listed on the customer plan page.
        if (isCustomer(userDetails)) {
            return ResponseEntity.ok(
                    planService.getPurchasablePlansByProductId(productId, page, size, sortBy, direction));
        }

        PaginatedResponseDTO<PolicyPlanResponseDTO> response = status != null
                ? planService.getPlansByProductIdAndStatus(
                        productId,
                        status,
                        page,
                        size,
    ...
    private boolean isCustomer(CustomUserDetails userDetails) {
        return userDetails != null && "CUSTOMER".equals(userDetails.getRole());
    }
}