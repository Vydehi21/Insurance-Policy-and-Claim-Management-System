package com.monocept.project.repository;
    ...
    public interface PolicyRepository extends JpaRepository<Policy, Long> {
        ...
        // Row lock used when recording a payment, so two simultaneous payments on
        // the same policy can't both succeed and double totalPremiumPaid.
        @Lock(LockModeType.PESSIMISTIC_WRITE)
        @Query("SELECT p FROM Policy p WHERE p.id = :policyId")
        Optional<Policy> findByIdForUpdate(@Param("policyId") Long policyId);

        boolean existsByPolicyPlan_Id(Long planId);
